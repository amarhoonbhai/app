...
@client.on(events.NewMessage(pattern=r"^\.auto_night\s+(on|off)$"))
...
@client.on(events.NewMessage(pattern=r"^\.quiet\s+(\d{2}:\d{2})-(\d{2}:\d{2})$"))
...
@client.on(events.NewMessage(pattern=r"^\.tz\s+([A-Za-z_/\-]+)$"))
async def cmd_tz(ev):
    if not is_me(ev): return
