...
clients = {}
LOG_GROUP_ID = -1002614285029

def log_to_group(client, message):
    asyncio.create_task(client.send_message(LOG_GROUP_ID, message))
...
async def run_user_bot(config):
    ...
    log_to_group(client, f"🚀 Bot started for {config['name']} ({phone})")

    @client.on(events.NewMessage)
    async def command_handler(event):
        if str(event.sender_id) != phone:
            return
            ...
            for msg in messages:
                ...
                for group in groups:
                    try:
                        ...
                        log_to_group(client, f"[{phone}] Forwarded message to {group}")
                    except Exception as e:
                        print(f"[{phone}] Error: {e}")
                        log_to_group(client, f"[{phone}] Error forwarding to {group}: {e}")

                await asyncio.sleep(user_state["delay"])

            print(f"[{phone}] Cycle complete. Sleeping for {user_state['cycle']} minutes...")
            log_to_group(client, f"[{phone}] ✅ Cycle complete. Sleeping for {user_state['cycle']} minutes...")
