...
- Dot-commands with outgoing=True (works in any chat you can send)
- Auto night mode (quiet window) + manual night + rest + delay
- Human durations for delay/time: 10, 10s, 30m, 2h
- Phone normalization + per-user JSON
"""
...
# === Runtime ===
runtime = {"forward_delay": 5, "groups": [], "night_mode": False, "rest_until": None}
...
def parse_duration_seconds(s: str) -> int:
    """
    Accepts: "30", "30s", "30m", "2h" (case-insensitive).
    Defaults to seconds if unit omitted.
    """
    s = s.strip()
    m = re.fullmatch(r"(\d+)([smhSMH]?)", s)
    if not m:
...
async def main(phone: str):
    ...
    @client.on(events.NewMessage(pattern=r"^\.delay\s+(\d+)([smhSMH]?)$", outgoing=True))
    async def cmd_delay(ev):
        if not is_me(ev): return
        amount = ev.pattern_match.group(1) + (ev.pattern_match.group(2) or "")
        try:
            secs = parse_duration_seconds(amount)
        except Exception:
            await ev.reply("Usage: .delay <N[s|m|h]>")
            return
        new_delay = max(secs, 5)
        runtime["forward_delay"] = new_delay
        save_runtime(phone, cfg)
        await ev.reply(f"⏱️ Delay set to {new_delay}s")

    @client.on(events.NewMessage(pattern=r"^\.time(?:\s+(\d+)([smhSMH]?))?$", outgoing=True))
    async def cmd_time(ev):
        if not is_me(ev): return
        g = ev.pattern_match.group(1)
        u = ev.pattern_match.group(2)
        if g:
            amount = g + (u or "")
            try:
                secs = parse_duration_seconds(amount)
            except Exception:
                await ev.reply("Usage: .time <N[s|m|h]> (e.g., .time 30m)")
                return
            new_delay = max(secs, 5)
            runtime["forward_delay"] = new_delay
            save_runtime(phone, cfg)
            ...
            await ev.reply(f"⏱️ Current delay: {runtime['forward_delay']}s")
    ...
    # Forward from Saved Messages (respect quiet/rest)
